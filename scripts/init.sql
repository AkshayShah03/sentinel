-- ============================================================
-- AI Safety Platform — Database Schema
-- ============================================================
-- Run automatically on first Postgres boot via Docker entrypoint

CREATE EXTENSION IF NOT EXISTS "uuid-ossp";
CREATE EXTENSION IF NOT EXISTS "pg_trgm";   -- for text search on prompt content
...
CREATE INDEX idx_prompts_name_status ON prompts(name, status);
...
CREATE TABLE prompt_audit (
    ...
    action      TEXT NOT NULL,   -- create | promote_success | promote_failed | rollback | deprecate
    actor       TEXT NOT NULL,
    metadata    JSONB NOT NULL DEFAULT '{}',
    created_at  TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
CREATE INDEX idx_prompt_audit_prompt_id ON prompt_audit(prompt_id);

-- ─── A/B tests ───────────────────────────────────────────────
CREATE TABLE ab_tests (
    test_id             UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    prompt_id           TEXT NOT NULL,    -- prompt name (not ID — spans versions)
    ...
    conclusion          TEXT             -- populated when test ends
);
CREATE INDEX idx_ab_tests_prompt_active ON ab_tests(prompt_id, active);
...
CREATE INDEX idx_ab_assignments_test_id ON ab_assignments(test_id);

-- ─── Eval queue (pending work) ───────────────────────────────
...
CREATE INDEX idx_eval_queue_status_priority ON eval_queue(status, priority, created_at);
...
CREATE INDEX idx_eval_results_prompt ON eval_results(prompt_id, prompt_version, evaluated_at DESC);
CREATE INDEX idx_eval_results_request_id ON eval_results(request_id);

-- ─── Guardrail log (every request + verdict) ────────────────
...
CREATE INDEX idx_guardrail_log_verdict ON guardrail_log(verdict, logged_at DESC);
CREATE INDEX idx_guardrail_log_request ON guardrail_log(request_id);

-- ─── Quality snapshots (TimescaleDB-style time series) ───────
...
CREATE INDEX idx_quality_snapshots_prompt_time
    ON quality_snapshots(prompt_id, prompt_version, window_end DESC);
...
CREATE INDEX idx_drift_alerts_prompt ON drift_alerts(prompt_id, triggered_at DESC);
CREATE INDEX idx_drift_alerts_unresolved ON drift_alerts(resolved_at) WHERE resolved_at IS NULL;

-- ─── CI gate results ─────────────────────────────────────────
CREATE TABLE ci_gate_results (
    id              BIGSERIAL PRIMARY KEY,
    prompt_id       UUID NOT NULL REFERENCES prompts(prompt_id),
    passed          BOOLEAN NOT NULL,
